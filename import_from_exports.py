...
def find_latest_export_files(exports_dir: Path) -> Dict[str, Optional[Path]]:
    ...
    """
    files = {"tweets": None, "notes": None, "media_metadata": None}
    ...
    for i in tqdm(range(0, len(df), batch_size), desc="Importing tweets"):
        ...
        for _, row in batch.iterrows():
            try:
                record = {
                    "tweet_id": (
                        int(row["tweet_id"]) if pd.notna(row["tweet_id"]) else None
                    ),
                    "text": row["text"] if pd.notna(row["text"]) else None,
                    "created_at": parse_datetime(row["created_at"]),
                    "author_id": (
                        row["author_id"] if pd.notna(row["author_id"]) else None
                    ),
                    "author_name": (
                        row["author_name"] if pd.notna(row["author_name"]) else None
                    ),
                    "author_username": (
                        row["author_username"]
                        if pd.notna(row["author_username"])
                        else None
                    ),
                    "author_verified": parse_boolean(row["author_verified"]),
                    "likes": int(row["likes"]) if pd.notna(row["likes"]) else None,
                    "retweets": (
                        int(row["retweets"]) if pd.notna(row["retweets"]) else None
                    ),
                    "replies": (
                        int(row["replies"]) if pd.notna(row["replies"]) else None
                    ),
                    ...
                    "tweet_url": (
                        row["tweet_url"] if pd.notna(row["tweet_url"]) else None
                    ),
            ...
            try:
                ...
                stmt = stmt.on_conflict_do_update(
                    ...
                    set_={
                        ...
                        "tweet_url": stmt.excluded.tweet_url,
    ...
    for i in tqdm(range(0, len(df), batch_size), desc="Importing notes"):
        ...
        for _, row in batch.iterrows():
            try:
                record = {
                    "note_id": (
                        int(row["note_id"]) if pd.notna(row["note_id"]) else None
                    ),
                    "tweet_id": (
                        int(row["tweet_id"]) if pd.notna(row["tweet_id"]) else None
                    ),
                    "note_author_participant_id": (
                        row["note_author_participant_id"]
                        if pd.notna(row["note_author_participant_id"])
                        else ""
                    ),
                    "created_at_millis": (
                        int(row["created_at_millis"])
                        if pd.notna(row["created_at_millis"])
                        else None
                    ),
                    "classification": (
                        row["classification"]
                    ...
                    "believable": (
                        row["believable"] if pd.notna(row["believable"]) else None
                    ),
                    "harmful": row["harmful"] if pd.notna(row["harmful"]) else None,
                    "validation_difficulty": (
                        row["validation_difficulty"]
                        if pd.notna(row["validation_difficulty"])
                        else None
                    ),
                    "misleading_other": (
                        int(row["misleading_other"])
                        if pd.notna(row["misleading_other"])
                        else None
                    ),
                    "misleading_factual_error": (
                        int(row["misleading_factual_error"])
                        if pd.notna(row["misleading_factual_error"])
                        else None
                    ),
                    "misleading_manipulated_media": (
                        int(row["misleading_manipulated_media"])
                        if pd.notna(row["misleading_manipulated_media"])
                        else None
                    ),
                    "misleading_outdated_information": (
                        int(row["misleading_outdated_information"])
                        if pd.notna(row["misleading_outdated_information"])
                        else None
                    ),
                    "misleading_missing_important_context": (
                        int(row["misleading_missing_important_context"])
                        if pd.notna(row["misleading_missing_important_context"])
                        else None
                    ),
                    "misleading_unverified_claim_as_fact": (
                        int(row["misleading_unverified_claim_as_fact"])
                        if pd.notna(row["misleading_unverified_claim_as_fact"])
                        else None
                    ),
                    "misleading_satire": (
                        int(row["misleading_satire"])
                        if pd.notna(row["misleading_satire"])
                        else None
                    ),
                    "not_misleading_other": (
                        int(row["not_misleading_other"])
                        if pd.notna(row["not_misleading_other"])
                        else None
                    ),
                    "not_misleading_factually_correct": (
                        int(row["not_misleading_factually_correct"])
                        if pd.notna(row["not_misleading_factually_correct"])
                        else None
                    ),
                    "not_misleading_outdated_but_not_when_written": (
                        int(row["not_misleading_outdated_but_not_when_written"])
                        if pd.notna(row["not_misleading_outdated_but_not_when_written"])
                        else None
                    ),
                    "not_misleading_clearly_satire": (
                        int(row["not_misleading_clearly_satire"])
                        if pd.notna(row["not_misleading_clearly_satire"])
                        else None
                    ),
                    "not_misleading_personal_opinion": (
                        int(row["not_misleading_personal_opinion"])
                        if pd.notna(row["not_misleading_personal_opinion"])
                        else None
                    ),
                    "trustworthy_sources": (
                        int(row["trustworthy_sources"])
                        if pd.notna(row["trustworthy_sources"])
                        else None
                    ),
                    "summary": row["summary"] if pd.notna(row["summary"]) else None,
                    "is_media_note": parse_boolean(row["is_media_note"]),
                    "note_url": row["note_url"] if pd.notna(row["note_url"]) else None,
                }
...
def import_media_metadata(
    session, csv_path: Path, batch_size: int = 500
) -> Dict[str, int]:
    """
        ...
        for _, row in batch.iterrows():
            try:
                record = {
                    ...
                    "uploader_id": (
                        row["uploader_id"] if pd.notna(row["uploader_id"]) else None
                    ),
                    "timestamp": (
                    ...
                    "like_count": (
                        int(row["like_count"]) if pd.notna(row["like_count"]) else None
                    ),
                    ...
                    "local_path": (
                        row["local_path"] if pd.notna(row["local_path"]) else None
                    ),
                }
...
def verify_import(session) -> Dict[str, int]:
    ...
    try:
        ...
        counts["media_metadata"] = session.execute(
            text("SELECT COUNT(*) FROM media_metadata")
        ).scalar()
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--skip-tweets", action="store_true", help="Skip importing tweets"
    )
    parser.add_argument(
        "--skip-notes", action="store_true", help="Skip importing notes"
    )
    parser.add_argument(
        "--skip-media", action="store_true", help="Skip importing media metadata"
    )
    ...
    try:
        ...
        if not args.skip_media and files["media_metadata"]:
            ...
            stats = import_media_metadata(
                session, files["media_metadata"], args.batch_size
            )
