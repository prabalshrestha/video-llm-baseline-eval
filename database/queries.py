...
def get_media_metadata_by_tweet_id(
    session: Session, tweet_id: int
    ...
    """
    return (
        session.query(MediaMetadata).filter(MediaMetadata.tweet_id == tweet_id).first()
    )
...
def get_misleading_media(
    ...
    query = (
        ...
        .filter(Note.classification == classification, Tweet.likes >= min_engagement)
    )
...
def get_media_by_note_id(session: Session, note_id: int) -> Optional[Dict[str, Any]]:
    ...
    if result:
        ...
        return {"note": note, "tweet": tweet, "media_metadata": media}
    return None


def get_evaluation_dataset(
    session: Session, limit: Optional[int] = None, classification: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
...
def filter_by_classification_and_media(
    ...
    """
    query = session.query(Note, Tweet, MediaMetadata).join(
...
def export_to_json_format(
    session: Session, output_limit: Optional[int] = None
) -> List[Dict[str, Any]]:
    """
    ...
    for note, tweet, media in query.all():
        sample = {
            "video": {
                "filename": (
                    media.local_path.split("/")[-1]
                    if media and media.local_path
                    else None
                ),
                "duration_seconds": (
                    media.duration_ms / 1000 if media and media.duration_ms else None
                ),
